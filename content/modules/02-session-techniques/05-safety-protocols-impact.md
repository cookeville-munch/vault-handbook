...
```mermaid
graph TD
    A[Initiate Scene] --> B{Participant Readiness?}
    B -->|Yes| C[Green Light Active]
    B -->|No| D[Adjust/Postpone]
    C --> E[Yellow Zone Transition]
    E --> F[Yellow Light Activation]
    F --> G[Adjust Intensity/Timing]
    C --> H[Proceed with Monitoring]
    H --> E
```
